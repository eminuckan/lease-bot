...
export function AgentAvailabilityPanel() {
  ...
  useEffect(() => {
    ...
    setRuleForm((current) => ({ ...current, timezone: user.timezone }));
  }, [user?.timezone]);
  ...
  const selectedDayAvailability = useMemo(
    () => availabilityItemsByDay.get(dateKey(selectedDate)) || [],
      ...
      <div className="mx-auto w-full max-w-[1400px] space-y-4">
        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-foreground">
            {agentWeeklyRules.length} recurring rules
          </span>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[2fr_1fr]">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-[10px] text-muted-foreground">{dayAvailability.length}</span>
                    </div>
                    <div className="mt-1 space-y-1">
                      {dayAvailability.slice(0, 2).map((item) => (
                        <div key={item.id} className="truncate rounded bg-emerald-500/10 px-1.5 py-0.5 text-[10px] text-emerald-300">
                          {format(safeDate(item.startsAt) || new Date(), "HH:mm")}
                        </div>
                      ))}
                    </div>
                  </button>
          ...
          <section className="space-y-3">
            <div className="rounded-md border border-border bg-card p-3">
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">Availability for selected day.</p>
              ...
              <div className="mt-3 space-y-2">
                ...
                {selectedDayAvailability.length === 0 ? (
                  <div className="rounded-md border border-dashed border-border px-3 py-6 text-center text-sm text-muted-foreground">
                    No availability slots for selected day.
                  </div>
                ) : null}
              </div>
            </div>
